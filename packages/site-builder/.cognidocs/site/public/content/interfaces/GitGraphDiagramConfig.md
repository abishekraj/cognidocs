# GitGraphDiagramConfig

This interface was referenced by `MermaidConfig`'s JSON-Schema via the `definition` "GitGraphDiagramConfig".

## Properties

| Name                  | Type        | Optional | Description |
| :-------------------- | :---------- | :------- | :---------- |
| `titleTopMargin`      | `number`    | Yes      | -           |
| `diagramPadding`      | `number`    | Yes      | -           |
| `nodeLabel`           | `NodeLabel` | Yes      | -           |
| `mainBranchName`      | `string`    | Yes      | -           |
| `mainBranchOrder`     | `number`    | Yes      | -           |
| `showCommitLabel`     | `boolean`   | Yes      | -           |
| `showBranches`        | `boolean`   | Yes      | -           |
| `rotateCommitLabel`   | `boolean`   | Yes      | -           |
| `parallelCommits`     | `boolean`   | Yes      | -           |
| `arrowMarkerAbsolute` | `boolean`   | Yes      | -           |
