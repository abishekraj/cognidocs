 ...
 * @param propName - Optional prop name for context-aware defaults
...
export function parseDefaultValue(
  ...
  if (!defaultValue) {
    return getDefaultForType(editorType, propName);
  }
 ...
 */
export function getDefaultForType(editorType: PropEditorType, propName?: string): unknown {
  switch (editorType) {
    case 'string':
      return '';
    case 'number':
    ...
    case 'function':
      return undefined;
    case 'react-node':
      // Provide meaningful default for children props
